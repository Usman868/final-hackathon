...
import { PriorityBadge, IssueStatusBadge } from '../../components/ui/StatusBadge';
import { pickData } from '../../utils/safe';
import toast from 'react-hot-toast';
import { cn } from '../../utils/cn';

const PIE_COLORS = ['#0f766e', '#0d9488', '#14b8a6', '#f59e0b', '#ef4444', '#64748b', '#8b5cf6'];
...
export default function ReportsPage() {
  ...
  const issuesByStatus = Object.entries(summary?.issuesByStatus || {}).map(([name, value]) => ({
    name,
    value: Number(value) || 0,
  }));
  const issuesByPriority = Object.entries(summary?.issuesByPriority || {}).map(([name, value]) => ({
    name,
    value: Number(value) || 0,
  }));
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      pdf.text(`Generated ${format(new Date(), 'MMM d, yyyy HH:mm')}`, margin, 13);
  ...
  if (!summary) {
    return (
      <div className="card p-10 text-center text-ink-500">
        ...
        <button type="button" onClick={load} className="mt-3 block w-full text-brand-700">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div ref={reportRef} className="space-y-5 rounded-xl bg-surface-muted p-1 sm:p-0">
        {/* KPI cards */}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="card p-4">
            <h2 className="mb-3 text-sm font-semibold text-ink-900">Issues by status</h2>
          ...
          <div className="card p-4">
            <h2 className="mb-3 text-sm font-semibold text-ink-900">Issues by priority</h2>
        ...
        {assetsByCategory.length > 0 && (
          <div className="card p-4">
            <h2 className="mb-3 text-sm font-semibold text-ink-900">Assets by category</h2>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={assetsByCategory} layout="vertical" margin={{ left: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
        ...
        <div className="card overflow-hidden">
          <div className="border-b border-border px-5 py-3">
            <h2 className="text-sm font-semibold text-ink-900">Critical / recent issues</h2>
          </div>
                  ...
                  (summary.criticalIssues?.length
                    ...
                    <tr key={iss._id || iss.issueNumber}>
                      <td className="px-5 py-2.5">
                        <p className="font-medium text-ink-900">{iss.issueNumber}</p>
                        <p className="text-xs text-ink-500 line-clamp-1">{iss.title}</p>
                      </td>
        ...
        {(summary.upcomingServices || []).length > 0 && (
          <div className="card overflow-hidden">
            <div className="border-b border-border px-5 py-3">
              <h2 className="text-sm font-semibold text-ink-900">Upcoming services (30 days)</h2>
            </div>
              ...
              {summary.upcomingServices.map((a) => (
                ...
                >
                  <span className="font-medium text-ink-800">
                    {a.name}{' '}
                    <span className="font-mono text-xs text-ink-400">{a.assetCode}</span>
                  </span>
